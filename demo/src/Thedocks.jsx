...
export default function Thedock({ configuration, ar,  onclickHandler, textstyles, marginForText, showText, texts, style,  iconWidth, maxBoxes,  containerHeight, setpercentage, percentage, containerWidth, iconstyles, icons, startingPosition }) {
    // alert('rerendered')
    ...
    const spawnPlaceDecider = (startingPosition, containerXstart, containerYstart, standard_width, standard_height) => {
        ...
        let directionx = 0;
        let directiony = 0;
        if (startingPosition === 'top-left' || !startingPosition) {
    ...
    React.useEffect(() => {
        ...
        let allIconsOnDestiny = icons.map((iconSRC, index) => {
            ...
            return {
                ...
                <div className="box" style={{...transStyles, ...stylings.box, height: boxHeight, width: boxwidth}}>
                    <img onClick={()=>(onclickHandler(index, iconSRC, text))} className="thedock_icon" style={{ ...stylings.icon,  width: boxwidth, maxWidth: iconWidth === 'auto' ? 'auto' : iconWidth - 0,  height: 'auto', ...iconstyles }} src={iconSRC} />
        ...
        console.log(allIconsDestiny)
        setAlignment(alignment)
        setAllIconsDestiny(allIconsOnDestiny)
        setheight(standard_height)


    }, [configuration, maxBoxes, icons, containerWidth, startingPosition ]);
